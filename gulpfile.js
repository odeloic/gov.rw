...
const srcPath = (file, watch = false) => {
  if (file === 'scss' && watch === false) return './src/stylesheets/main.scss';
  if (file === 'scss' && watch === true) return './src/stylesheets/**/*.scss';
  if (file === 'js' && watch === false) return entryArray;
  if (file === 'js' && watch === true) return './src/scripts/**/*.js';
  if (file === 'html') return './src/*.html';
  if (file === 'img') return './src/assets/**/*.{png,jpeg,jpg,svg,gif}';
...
const cleanExport = (mode) => () => {
  return ['development', 'production'].includes(mode) ? del(['..zip']) : undefined;
};
